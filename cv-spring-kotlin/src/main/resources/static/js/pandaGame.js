(function () {
    ...
    const WITCH_HEIGTH_SPAWN_MAX  = 220;
    const WITCH_HEIGTH_SPAWN  = 120;
    const WITCH_HEIGTH_SPAWN_MIN  = 50;
    ...
    function spawnWitch() {
        var witchHeigth = WITCH_HEIGTH_SPAWN;
        const rand = Math.floor(Math.random() * 3);
        if(rand === 0)
            witchHeigth = Math.random() * (WITCH_HEIGTH_SPAWN_MAX - WITCH_HEIGTH_SPAWN_MIN) + WITCH_HEIGTH_SPAWN_MIN;

        spawner.obstacles.push({ type: 'witch', x: WIDTH + 55, y: witchHeigth, anim: 0 });
    }
